...
class SupabaseCrudService {
  ...
  Future<void> updateMaterial(LearningMaterial material) async {
    await client.from('learning_materials').update({
      'title': material.title,
      'subject': material.subject,
      'description': material.description,
      'file_url': material.fileUrl,
  ...
  Future<List<User>> fetchUsers() async {
    ...
    return (response as List)
        .map((e) => User(
              ...
              recentActivity: (e['recent_activity'] as List?)?.cast<String>() ?? [],
            ))
        .toList();
  }
  ...
  Future<List<Library>> fetchLibraries() async {
    ...
    return (response as List)
        .map((e) => Library(
              ...
              address: e['address'],
              hours: e['hours'],
              availableBooks: (e['available_books'] as List?)?.cast<String>() ?? [],
  ...
  Future<void> updateLibrary(Library library) async {
    await client.from('libraries').update({
      'name': library.name,
      'address': library.address,
      'hours': library.hours,
      'available_books': library.availableBooks,
