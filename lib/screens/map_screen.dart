...
class _MapScreenState extends State<MapScreen> {
  ...
  double _distanceFromNairobi(double lat, double lng) {
    ...
    final a =
        ...
        cos(_deg2rad(_nairobiLat)) * cos(_deg2rad(lat)) *
  ...
  List<String> get _allCounties {
    final List<String> counties = _allLibraries
        .map((lib) => lib['county'])
        .where((c) => c != null && (c as String).isNotEmpty)
        .map((c) => c as String)
        .toSet()
        .toList();
    counties.sort();
  ...
  List<String> get _allTypes {
    final List<String> types = _allLibraries
        .map((lib) => lib['type'])
        .where((t) => t != null && (t as String).isNotEmpty)
        .map((t) => t as String)
        .toSet()
        .toList();
    types.sort();
  ...
  List<Map<String, dynamic>> get _filteredLibraries {
    return _allLibraries.where((lib) {
      final matchesQuery =
          ...
          lib['name'].toString().toLowerCase().contains(
